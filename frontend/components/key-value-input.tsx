...
export default function KeyValueEditor({
    ...
    const addRow = () =>
        setRows((r) => [...r, { id: Date.now(), field: "", info: "" }])
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, id: number) => {
        if (e.key === "Enter") {
            e.preventDefault()
            // If current row has content, add a new one
        ...
        <div className="space-y-3">
            ...
            <div className="space-y-2">
                {rows.map((r) => (
                    <div
                        key={r.id}
                        className="group grid grid-cols-[1fr_3fr_max-content] gap-2 items-center"
                    >
                        <Input
                        ...
                        <Button
                            ...
                            title="Remove row"
                            className="opacity-0 group-hover:opacity-100 transition-opacity justify-self-end size-8"
                        >
